<?php
    ...
    // Count featured listings for the Saved Properties counter
    $featured_count_query = "SELECT COUNT(*) AS featured_count FROM featured_listings";
    $featured_count_result = mysqli_query($connect, $featured_count_query);
    $featured_count = 0;
    if ($featured_count_result && $row = mysqli_fetch_assoc($featured_count_result)) {
        $featured_count = (int)$row['featured_count'];
    }

    // Count active (pending) applications for the Active Applications counter
    $applications_count_query = "SELECT COUNT(*) AS applications_count FROM reservations WHERE status = 'pending'";
    $applications_count_result = mysqli_query($connect, $applications_count_query);
    $applications_count = 0;
    if ($applications_count_result && $row = mysqli_fetch_assoc($applications_count_result)) {
...
<body>
    ...
    <div class="admin-container">
        <div class="admin-sidebar">
            <div class="admin-menu-item active" data-section="dashboard" style="margin-top:90px;">
                ...
                <div class="admin-stats">
                    <div class="stat-card">
                        ...
                        <div class="stat-info">
                            ...
                            <div class="stat-number" id="saved-count"><?php echo $featured_count; ?></div>
                        </div>
                    </div>
                    <div class="stat-card">
                        <div class="stat-icon">
    ...
    <script>
        ...
        function handleRequest(requestId, action) {
            ...
            .then(data => {
                showNotification(data.message, data.success ? 'info' : 'error');
                // Notify user for both accept and decline
                if (data.success && data.user_email && data.user_name && data.listing_title) {
                    const templateParams = {
                        ...
                        message: action === 'accept' 
                            ? `Your listing "${data.listing_title}" has been accepted and is now featured!`
                            : `Your listing "${data.listing_title}" has been declined.`
                    };
                    
                    emailjs.send("service_wn8muwd", "template_1gueejj", templateParams)
                        .then(() => {
                            showNotification(`User notified about ${action}ed listing.`, 'success');
                        })
                        .catch((err) => {
                            console.error('EmailJS error:', err);
                            showNotification('Failed to send email notification to user.', 'error');
